...
window.completeOfflineCheckbox = function () {
  ...
  for (var i = 0; i < offlineInputs.length; i++) {
	...
	jQuery.post(ajaxurl, {
		...
		'public_key': jQuery('#woocommerce_woo-mercado-pago-ticket__mp_public_key_prod').val(),
		'access_token': jQuery('#woocommerce_woo-mercado-pago-ticket__mp_access_token_prod').val()
	}, function (response) {
		if (response.success) {
			jQuery('#woocommerce_woo-mercado-pago-ticket__mp_public_key_prod').removeClass("mp_credential_input_loading").addClass("mp_credential_input_success");
			jQuery('#woocommerce_woo-mercado-pago-ticket__mp_access_token_prod').removeClass("mp_credential_input_loading").addClass("mp_credential_input_success");
		}
	...
	jQuery.post(ajaxurl, {
		'action': 'mercadopago_validate_credentials',
		'public_key': jQuery('#woocommerce_woo-mercado-pago-ticket__mp_public_key_test').val(),
		'access_token': jQuery('#woocommerce_woo-mercado-pago-ticket__mp_access_token_test').val()
	}, function (response) {
		if (response.success) {
			jQuery('#woocommerce_woo-mercado-pago-ticket__mp_public_key_test').removeClass("mp_credential_input_loading").addClass("mp_credential_input_success");
			jQuery('#woocommerce_woo-mercado-pago-ticket__mp_access_token_test').removeClass("mp_credential_input_loading").addClass("mp_credential_input_success");
		}
		console.log('Got this from the server: ' + JSON.stringify(response));
	});
});
