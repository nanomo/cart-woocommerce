...
class MP {
    ...
    foreach ( $response['response'] as $key => $value ) {	
			$response['response'][$key]['payment_places'] =  $this->build_payment_places($value['id']); 
	 ...
	 * @param $lpayment_id
	 */
	public function build_payment_places($payment_id) {
    ...
    if (array_key_exists($payment_id, $payment_places)) {
            return $payment_places[$payment_id];
  }
