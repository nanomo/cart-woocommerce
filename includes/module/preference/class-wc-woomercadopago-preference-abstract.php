...
abstract class WC_WooMercadoPago_Preference_Abstract extends WC_Payment_Gateway {
	...
	public function get_internal_metadata() {
		...
		return array(
			...
			'test_mode'        => $test_mode,
			'details'          => '',
			'basic_settings'   => wp_json_encode( $analytics->getBasicSettings(), true ),
			'custom_settings'  => wp_json_encode( $analytics->getCustomSettings(), true ),
			'ticket_settings'  => wp_json_encode( $analytics->getTicketSettings(), true ),
		);
	}
