												...
												<?php foreach ( $payment['payment_places'] as $place ) : ?>
													<div id="frameTicket" class="mp-col-md-6 mp-pb-15 mp-min-hg">
														...
														<label for="<?php echo esc_attr($payment['id']); ?>" class="mp-label-form mp-pointer">
															<input type="radio" class="mp-form-control-check" name="mercadopago_ticket[paymentMethodId]" id="<?php echo esc_attr($payment['id']); ?>" value="<?php echo esc_attr($payment['id'] . '|' . $place['payment_option_id'] ); ?>"
															<?php if ( $at_first ) : ?>
